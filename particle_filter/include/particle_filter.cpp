...
ParticleFilter::ParticleFilter(int p_Num, int sensor_LineNum, int map_H, int map_W)
{
    pNum = p_Num;
...
void ParticleFilter::build_LikelihoodMap()
{
    ...
    cv::Mat gray_img;
    cv::Mat binary_img;
    likelihood_map=new cv::Mat(cv::Size(600,600),CV_8UC1,Scalar(0));
    ...
    imwrite("/home/iclab/catkin_ws/devel/lib/particle_filter/likelihood_img.jpg",*likelihood_map);
    //waitKey(60);
}
...
{
    double nowDeg = 0;
    double eachDeg = (180.0/sensorLineNum);
    double nowRad;
...
Vector3i ParticleFilter::get_Estimate_pos()
{
    Vector3i grid_pos(predictionPos(0),predictionPos(1),predictionPos(2));
    return grid_pos;
}
